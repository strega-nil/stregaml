...
let is_ident_continue_no_prime ch =
  is_ident_start ch || is_number_start ch

let is_ident_continue ch =
  is_ident_continue_no_prime ch || ch = '\''
...
let peek_ch lex =
  if String.length lex.buffer <= lex.index then
    None
  else
    Some (lex.buffer.[lex.index], current_span lex)
...
let next_ch lex =
  ...
  | Some (ch, sp) ->
    ...
    if ch = '\n' then
      (lex.line <- lex.line + 1; lex.column <- 1)
    else
...
let lex_number fst sp lex =
  ...
  let rec helper idx sp space_allowed =
    ...
    | Some (ch, sp')
      when space_allowed
        && (is_number_continue ch 10 || is_ident_continue ch) ->
      eat_ch lex;
      S.push ch buff;
