...
type t = {
  lexer: Lex.t;
  mutable peek: (Token.t * span) option;
}
...
let wrap_sok
: ('o, 'e) spanned_result -> ('o spanned, 'e) spanned_result
= function
| Ok (o, sp) -> Ok ((o, sp), sp)
| Error e -> Error e
...
type item =
| Item_func of Ast.Function.builder
...
let rec maybe_parse_expression parser =
  let initial =
    ...
    | _ -> wrap None
  in
  match%bind  wrap_opt_sok initial with
  | Some i ->
...
and parse_follow parser (initial, sp) =
  ...
  let module M = Result.Monad(struct type t = Error.t spanned end) in
  let open! M in
  let%bind (tok, tok_sp) = peek_token parser in
  let%bind (initial, cont, sp) = match tok with
  | Token.Open_paren ->
...
and parse_parameter_list parser =
  ...
  let rec get_parms comma =
    ...
    | Token.Comma ->
      if comma then
        (eat_token parser; get_parms false)
      else
        wrap_err (
